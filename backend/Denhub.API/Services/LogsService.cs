...
namespace Denhub.API.Services {
    public class LogsService : ILogsService {
        ...
        public async Task<ValueResult<TwitchChatMessagesResult>> GetByChannelAsync(long channelId, DateTime timestamp,
            ...
            var messagesResult = await GetByChannelIdAsync(channelId, timestamp, paginationCursor, limit);
            return Result.Ok(messagesResult);
        }

        public async Task<ValueResult<TwitchChatMessagesResult>> GetByChannelAsync(string channelName, DateTime timestamp,
            string paginationCursor = null, int limit = 100) {
            ...
            var messagesResult = await GetByChannelIdAsync(Convert.ToInt64(channelId), timestamp, paginationCursor, limit);
            return Result.Ok(messagesResult);
        }

        private async Task<TwitchChatMessagesResult> GetByChannelIdAsync(long channelId, DateTime timestamp,
            string paginationCursor, int limit) {
            var modifiedTimestamp = timestamp.Year < 1970 ? DateTime.UnixEpoch : timestamp;
            var (cursor, chatMessages) =
                await _chatLogsRepository.GetByChannelIdAsync(channelId, modifiedTimestamp, paginationCursor, limit);
